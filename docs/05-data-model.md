...
| `invited_by` | uuid, nullable FK → users | Event owner/admin who invited or approved the user |
...
| `id` | uuid, PK | |
| `event_id` | uuid, FK → events | |
| `uploaded_by` | uuid, nullable FK → users | Null for anonymous uploaders |
| `uploader_name` | varchar(100), nullable | Display name shown beside photo sharing controls |
| `cloudinary_url` | text | Cloudinary delivery URL |
| `cloudinary_id` | text | Used for asset deletion |
...
| `id` | uuid, PK | |
| `event_id` | uuid, FK → events | |
| `created_by` | uuid, nullable | Null for anonymous uploaders |
| `uploader_name` | varchar(100), nullable | Typed display name for anonymous uploaders |
| `total_files` | int | |
| `uploaded_files` | int | |
| `indexed_files` | int | |
| `failed_files` | int | |
| `status` | enum | `queued` / `uploading` / `indexing` / `completed` / `failed` |
